...
#define VIRTIO_GPU_EDID_SIZE	512
...
/*
 * Command: VIRTIO_GPU_CMD_FLUSH_SPRITE
 */
...
struct virtio_gpu_set_misc {
	...
	uint32_t scanout_id;
        ...
        uint32_t cmd_cnt;
        uint32_t padding;
        struct virtio_gpu_cmd cmd[MAX_SUPPORT_CMD];
};
...
static struct virtio_ops virtio_gpu_ops = {
	.name = "virtio-gpu",			/* our name */
	.cfgsize = sizeof(struct virtio_gpu_config),/* config reg size */
	.reset = virtio_gpu_reset,		/* reset */
	.qnotify = NULL,				/* device-wide qnotify */
	.cfgread = virtio_gpu_cfgread,		/* read PCI config */
	.cfgwrite = virtio_gpu_cfgwrite,		/* write PCI config */
	.apply_features = virtio_gpu_neg_features,	/* apply negotiated features */
	.set_status = virtio_gpu_set_status,		/* called on guest set status */
};
...
void
insert_vblank_irq(void *data,unsigned int frame,int i)
{
	struct virtio_gpu *gpu = (struct virtio_gpu *)data;
	struct virtio_vq_info *vq;
	struct iovec iov;
	uint32_t ret_value = frame;
	int n;
	uint16_t idx;

	vq = &gpu->vq[i];
	if(vq_disallow_interrupt(vq))
		return;
	if(vq_has_descs(vq)) {

		n = vq_getchain(vq, &idx, &iov, 1, NULL);
		if (n < 0) {
			pr_err("virtio-input: invalid descriptors\n");
			return;
		}
		if (n == 0) {
			pr_err("virtio-input: get no available desciptors\n");
			return;
		}
		if (n != 1) {
			pr_err("virtio_input: get wrong number of available descriptors\n");
			vq_relchain(vq, idx, 4); /* Release the chain */
			return;
		}
		memcpy(iov.iov_base, &ret_value, 4);
		vq_relchain(vq, idx, 4);
		vq_endchains(vq, 1);
...
static void virtio_gpu_dmabuf_unref(struct dma_buf_info *info)
{
	...
	if (atomic_sub_fetch(&info->ref_count, 1) == 0) {
		if (info->dmabuf_fd > 0) {
			vdpy_destroy_res(info->dmabuf_fd);
...
virtio_gpu_reset(void *vdev)
{
	...
	pr_dbg("Resetting virtio_gpu device.\n");
	gpu = vdev;
...
static inline bool test_feature_bit(uint64_t negotiated_caps, unsigned int fbit)
{
	return negotiated_caps & (1UL << fbit);
}
...
virtio_gpu_neg_features(void *vdev, uint64_t negotiated_features)
{
	...
	if(!test_feature_bit(gpu->base.negotiated_caps, VIRTIO_GPU_F_MULTI_PLANE)) {
		if(vdpy_mplane_check()) {
			pr_err("feature VIRTIO_GPU_F_MULTI_PLANE is not supported by guest \r\n");
			vdpy_mplane_fallback();
		}
	}
	...
	if(!test_feature_bit(gpu->base.negotiated_caps, VIRTIO_GPU_F_ROTATION))
		if(vdpy_mplane_check())
			pr_err("feature VIRTIO_GPU_F_ROTATION is not supported by guest \r\n");
...
virtio_gpu_cmd_get_edid(struct virtio_gpu_command *cmd)
{
	...
	if (req.scanout >= gpu->vdpy_if.scanout_num) {
...
static void
virtio_gpu_cmd_get_rotation(struct virtio_gpu_command *cmd)
{
	struct virtio_gpu_cmd_get_plane_rotation req;
	...
	memset(&resp, 0, sizeof(resp));
	virtio_gpu_update_resp_fence(&cmd->hdr, &resp.hdr);
	vdpy_get_plane_rotation(gpu->vdpy_handle, req.scanout_id, req.plane_id, resp.rotation, &resp.count);
	memcpy(cmd->iov[1].iov_base, &resp, sizeof(resp));
}

static void
virtio_gpu_cmd_get_planes_info(struct virtio_gpu_command *cmd)
{
	struct virtio_gpu_cmd_get_planes req;
	...
	memset(&resp, 0, sizeof(resp));
	virtio_gpu_update_resp_fence(&cmd->hdr, &resp.hdr);
	vdpy_get_plane_info(gpu->vdpy_handle, req.scanout, &resp.size, &resp.plane_num, resp.info);
	pr_info(" enable %d sprite planes for scanoutid:%d \r\n", resp.plane_num, req.scanout);
	memcpy(cmd->iov[1].iov_base, &resp, sizeof(resp));
}
...
virtio_gpu_cmd_get_display_info(struct virtio_gpu_command *cmd)
{
	...
	virtio_gpu_update_resp_fence(&cmd->hdr, &resp.hdr);
	for (i = 0; i < gpu->vdpy_if.scanout_num; i++) {
...
virtio_gpu_cmd_set_scanout(struct virtio_gpu_command *cmd)
{
	...
	memset(&resp, 0, sizeof(resp));
	virtio_gpu_update_resp_fence(&cmd->hdr, &resp);
	if (req.scanout_id >= gpu->vdpy_if.scanout_num) {
		pr_err("%s: Invalid scanout_id %d\n", req.scanout_id);
	...
	if ((req.r.x > r2d->width) ||
		...
		surf.stride[0] = pixman_image_get_stride(r2d->image);
		surf.surf_format = r2d->format;
		surf.surf_type = SURFACE_PIXMAN;
		surf.pixel += bytes_pp * surf.x + surf.y * surf.stride[0];
...
static void
virtio_gpu_cmd_flush_sync(struct virtio_gpu_command *cmd)
{
	struct virtio_gpu_flush_sync req;
	struct virtio_gpu_ctrl_hdr resp;
	struct virtio_gpu *gpu;
	...
	vdpy_sprite_flush_sync(gpu->vdpy_handle, req.scanout_id);
	resp.type = VIRTIO_GPU_RESP_OK_NODATA;
	memcpy(cmd->iov[1].iov_base, &resp, sizeof(resp));
	return;
...
static void
virtio_gpu_cmd_flush_sprite(struct virtio_gpu_command *cmd)
{
	struct virtio_gpu_flush_sprite req;
	struct virtio_gpu_ctrl_hdr resp;
	struct virtio_gpu_resource_2d *r2d = NULL;
	struct surface surf;
	struct virtio_gpu *gpu;
	int i;
	gpu = cmd->gpu;
	...
	memset(&resp, 0, sizeof(resp));
	virtio_gpu_update_resp_fence(&cmd->hdr, &resp);
	for(i=0; i<req.resource_cnt; i++) {
		r2d = virtio_gpu_find_resource_2d(gpu, req.resource_id[i]);
		if (r2d == NULL) {
			pr_err("%s: Illegal resource id %d\n", __func__,
					req.resource_id);
			resp.type = VIRTIO_GPU_RESP_ERR_INVALID_RESOURCE_ID;
		...
		if(i==0)
			...
			surf.dma_info.dmabuf_planar_fd[i -1] = r2d->dma_info->dmabuf_fd;

	}
	surf.dma_info.dmabuf_planar_fd_cnt = i-1;
	surf.fb_width = req.width;
	surf.fb_height = req.height;
	surf.width = req.r.width;
	surf.height = req.r.height;
	surf.x = req.r.x;
	surf.y = req.r.y;
	...
	for(int i=0; i<4; i++) {
		surf.stride[i] = req.strides[i];
		surf.offset[i] = req.offsets[i];
	}
	surf.modifier = req.modifier;
	surf.dma_info.surf_fourcc = req.format;
	virtio_gpu_dmabuf_ref(r2d->dma_info);
	vdpy_update_sprite(gpu->vdpy_handle, req.scanout_id, req.plane_id, &surf);
	virtio_gpu_dmabuf_unref(r2d->dma_info);
	resp.type = VIRTIO_GPU_RESP_OK_NODATA;
	memcpy(cmd->iov[1].iov_base, &resp, sizeof(resp));
	return;
...
virtio_gpu_cmd_create_blob(struct virtio_gpu_command *cmd)
{
	...
	if (req.nr_entries > 0) {
		...
		if (req.size > CURSOR_BLOB_SIZE) {
			...
			vdpy_create_res(r2d->dma_info->dmabuf_fd);
			r2d->blob = true;
		} else {
...
virtio_gpu_cmd_set_modifier(struct virtio_gpu_command *cmd)
{
	...
	cmd->iolen = sizeof(resp);
	memset(&resp, 0, sizeof(resp));
	vdpy_set_modifier(gpu->vdpy_handle, req.modifier, req.scanout_id);
	resp.type = VIRTIO_GPU_RESP_OK_NODATA;
	memcpy(cmd->iov[cmd->iovcnt - 1].iov_base, &resp, sizeof(resp));
	return;
}
...
static void
virtio_gpu_cmd_set_scaling(struct virtio_gpu_command *cmd)
{
	struct virtio_gpu_set_scaling req;
	struct virtio_gpu_ctrl_hdr resp;
	struct virtio_gpu *gpu;
	...
	cmd->iolen = sizeof(resp);
	memset(&resp, 0, sizeof(resp));
	vdpy_set_scaling(gpu->vdpy_handle, req.scanout_id, 0, req.dst.x, req.dst.y, req.dst.width, req.dst.height);
	resp.type = VIRTIO_GPU_RESP_OK_NODATA;
	memcpy(cmd->iov[cmd->iovcnt - 1].iov_base, &resp, sizeof(resp));
	return;
}
...
virtio_gpu_cmd_set_scanout_blob(struct virtio_gpu_command *cmd)
{
	...
	surf.dma_info.dmabuf_offset = req.offsets[0] + bytes_pp * surf.x + surf.y * surf.stride[0];
...
static void dispatch_tunnel_cmd(struct virtio_gpu *gpu, int handle, uint32_t scanout_id, uint32_t plane_id, struct virtio_gpu_cmd *cmdp)
{

	switch (cmdp->cmd) {

	case VIRTIO_GPU_TUNNEL_CMD_SET_ROTATION:
		vdpy_set_rotation(handle, scanout_id, plane_id, cmdp->data64[0]);
		break;
	case VIRTIO_GPU_TUNNEL_CMD_SET_BLEND:
		vdpy_set_pixel_blend_mode(handle, scanout_id, plane_id, cmdp->data32[0],(uint16_t)cmdp->data32[1]);
		break;
	case VIRTIO_GPU_TUNNEL_CMD_SET_PLANARS:
		uint32_t dmabuf_planar[3];
		struct virtio_gpu_resource_2d *r2d;
		int i;
		for(i=0; i<cmdp->size; i++) {
			r2d = virtio_gpu_find_resource_2d(gpu, cmdp->data32[i]);
			if(!r2d)
				pr_err("cannot find the rd dmabuf fd?--i:%d-------\r\n",i);

			dmabuf_planar[i] = r2d->dma_info->dmabuf_fd;
		}
		vdpy_set_planar(handle, scanout_id, plane_id, cmdp->size, dmabuf_planar);

		break;
	case VIRTIO_GPU_TUNNEL_CMD_SET_SPRITE_SCALING:
		vdpy_set_scaling(handle, scanout_id, plane_id, cmdp->data32[0], cmdp->data32[1], cmdp->data32[2], cmdp->data32[3]);
		break;
	default:
		pr_err("not allowable tunnel cmd \r\n");
		break;
...
static void virtio_gpu_cmd_set_misc(struct virtio_gpu_command *cmd)
{
	struct virtio_gpu_set_misc req;
	struct virtio_gpu_ctrl_hdr resp;
	struct virtio_gpu *gpu;
	int i;
	...
	cmd->iolen = sizeof(resp);
	memset(&resp, 0, sizeof(resp));
	for(i=0; i<req.cmd_cnt; i++) {
		struct virtio_gpu_cmd *cmdp = &req.cmd[i];
		dispatch_tunnel_cmd(gpu, gpu->vdpy_handle, req.scanout_id, req.plane_id, cmdp);
	}
...
virtio_gpu_ctrl_bh(void *data)
{
	...
	while (vq_has_descs(vq)) {
		...
		case VIRTIO_GPU_CMD_GET_DISPLAY_INFO:
			...
			break;
		case VIRTIO_GPU_CMD_GET_PLANES:
			virtio_gpu_cmd_get_planes_info(&cmd);
			break;
		...
		case VIRTIO_GPU_CMD_RESOURCE_FLUSH:
			...
			break;
		case VIRTIO_GPU_CMD_SET_MISC:
			virtio_gpu_cmd_set_misc(&cmd);
			break;
		case VIRTIO_GPU_CMD_FLUSH_SPRITE:
			virtio_gpu_cmd_flush_sprite(&cmd);
			break;
		case VIRTIO_GPU_CMD_FLUSH_SYNC:
			virtio_gpu_cmd_flush_sync(&cmd);
			break;
		...
		case VIRTIO_GPU_CMD_BACKLIGHT_UPDATE_STATUS:
			...
			break;
		case VIRTIO_GPU_CMD_GET_PLANE_ROTATION:
			virtio_gpu_cmd_get_rotation(&cmd);
			break;
...
static void
virtio_gpu_notify_irqqueue(void *vdev, struct virtio_vq_info *vq)
{
}
...
virtio_gpu_vga_bh(void *param)
{
	...
	if ((gpu->vga.surf.width != gpu->vga.gc->gc_image->width) ||
		...
		gpu->vga.surf.height = gpu->vga.gc->gc_image->height;
		gpu->vga.surf.stride[0] = gpu->vga.gc->gc_image->width * 4;
	        gpu->vga.surf.pixel = gpu->vga.gc->gc_image->data;
		gpu->vga.surf.surf_format = PIXMAN_a8r8g8b8;
		gpu->vga.surf.surf_type = SURFACE_PIXMAN;

		vdpy_surface_set_vga(gpu->vdpy_handle, 0, &gpu->vga.surf);
	}

	vdpy_surface_update_vga(gpu->vdpy_handle, 0, &gpu->vga.surf);
}
...
virtio_gpu_init(struct vmctx *ctx, struct pci_vdev *dev, char *opts)
{
	...
	memset(&gpu->vdpy_if, 0, sizeof(struct vdpy_if));
	gpu->vdpy_handle = vdpy_init(&gpu->vdpy_if,insert_vblank_irq,gpu);
	virtio_gpu_ops.nvq = gpu->vdpy_if.pipe_num + 2;

	gpu->vq = calloc(virtio_gpu_ops.nvq, sizeof(struct virtio_vq_info));
	...
	gpu->gpu_scanouts = calloc(gpu->vdpy_if.scanout_num, sizeof(struct virtio_gpu_scanout));
	...
	if (vm_allow_dmabuf(gpu->base.dev->vmctx)) {
		...
		if (gpu->is_blob_supported) {
			...
			gpu->base.device_caps |= (1UL << VIRTIO_GPU_F_MODIFIER);
			gpu->base.device_caps |= (1UL << VIRTIO_GPU_F_PIXEL_BLEND_MODE);

			if(vdpy_mplane_check()) {
				gpu->base.device_caps |= (1UL << VIRTIO_GPU_F_MULTI_PLANE);
				gpu->base.device_caps |= (1UL << VIRTIO_GPU_F_ROTATION);
				gpu->base.device_caps |= (1UL << VIRTIO_GPU_F_MULTI_PLANAR_FORMAT);
			}
		}
	}
	...
	for(i=2; i< 2 + gpu->vdpy_if.pipe_num; i++) {
		...
		gpu->vq[i].notify = virtio_gpu_notify_irqqueue;

		gpu->irq_bh[i-2].task_cb = virtio_gpu_irq_bh;
