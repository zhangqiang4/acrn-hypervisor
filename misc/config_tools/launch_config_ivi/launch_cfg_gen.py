...
class LaunchScript:
    ...
    def __init__(self, board_etree, vm_name, vm_scenario_etree, use_dgpu_sriov=False):
        ...
        self._use_dgpu_sriov = use_dgpu_sriov
...
def generate_for_one_vm(board_etree, hv_scenario_etree, vm_scenario_etree, vm_id, use_dgpu_sriov=False):
    vm_name = eval_xpath(vm_scenario_etree, "./name/text()", f"ACRN Post-Launched VM")
    script = LaunchScript(board_etree, vm_name, vm_scenario_etree, use_dgpu_sriov=use_dgpu_sriov)
    ...
    if eval_xpath(vm_scenario_etree, ".//own_pcpu/text()") == "y":
    ...
    for ivshmem in eval_xpath_all(vm_scenario_etree, f"//IVSHMEM_REGION[PROVIDED_BY = 'Device Model' and .//VM_NAME = '{vm_name}']"):
        ...
        if ivshmem.find('IVSHMEM_REGION_ID') is not None:
            script.add_virtual_device(f"ivshmem", str(slot) + ":" + str(func), options=f"dm:/{ivshmem.find('NAME').text},{ivshmem.find('IVSHMEM_SIZE').text},{ivshmem.find('IVSHMEM_REGION_ID').text}")
        else:
    ...
    for ivshmem in eval_xpath_all(vm_scenario_etree, f"//IVSHMEM_REGION[PROVIDED_BY = 'Hypervisor' and .//VM_NAME = '{vm_name}']"):
        ...
        if ivshmem.find('IVSHMEM_REGION_ID') is not None:
            ...
            script.add_virtual_device(f"ivshmem", str(slot) + ":" + str(func), options=f"hv:/{ivshmem.find('NAME').text},{ivshmem.find('IVSHMEM_SIZE').text},{0}")
...
def main(board_xml, scenario_xml, user_vm_id, out_dir, use_dgpu_sriov: bool = False):
    ...
    for post_vm in post_vms:
        ...
        script = generate_for_one_vm(board_etree, hv_scenario_etree, post_vm, post_vm_id, use_dgpu_sriov=use_dgpu_sriov)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--use-dgpu-sriov", type=int, default=0,
                        help="create VFs and set parameters for dGPU")
    parser.add_argument("--out", default="output", help="path to the directory where generated scripts are placed")
    args = parser.parse_args()

    logging.basicConfig(level="INFO")
    sys.exit(main(args.board, args.scenario, args.user_vmid, args.out, use_dgpu_sriov=bool(args.use_dgpu_sriov)))
